"""Type stubs for the Gingo C++ extension module."""

from typing import Any, Dict, Iterator, List, Optional, Tuple, Union, overload
from enum import Enum
from pathlib import Path

from gingo.audio import Envelope, Waveform

class ScaleType(Enum):
    Major: int
    NaturalMinor: int
    HarmonicMinor: int
    MelodicMinor: int
    Diminished: int
    HarmonicMajor: int
    WholeTone: int
    Augmented: int
    Blues: int
    Chromatic: int

class Modality(Enum):
    Diatonic: int
    Pentatonic: int

class Note:
    def __init__(self, name: str) -> None: ...
    def name(self) -> str: ...
    def natural(self) -> str: ...
    def sound(self) -> str: ...
    def semitone(self) -> int: ...
    def frequency(self, octave: int = 4, tuning: float = 440.0) -> float: ...
    def is_enharmonic(self, other: "Note") -> bool: ...
    def transpose(self, semitones: int) -> "Note": ...
    def distance(self, other: "Note") -> int: ...
    @staticmethod
    def fifths() -> List[str]: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __hash__(self) -> int: ...
    def __lt__(self, other: "Note") -> bool: ...
    def __le__(self, other: "Note") -> bool: ...
    def __gt__(self, other: "Note") -> bool: ...
    def __ge__(self, other: "Note") -> bool: ...
    def __int__(self) -> int: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __float__(self) -> float: ...
    @overload
    def __add__(self, semitones: int) -> "Note": ...
    @overload
    def __add__(self, interval: "Interval") -> "Note": ...
    def __radd__(self, semitones: int) -> "Note": ...
    @overload
    def __sub__(self, semitones: int) -> "Note": ...
    @overload
    def __sub__(self, interval: "Interval") -> "Note": ...
    @overload
    def __sub__(self, other: "Note") -> "Interval": ...

class Interval:
    @overload
    def __init__(self, label: str) -> None: ...
    @overload
    def __init__(self, semitones: int) -> None: ...
    @overload
    def __init__(self, from_note: Note, to_note: Note) -> None: ...
    def label(self) -> str: ...
    def anglo_saxon(self) -> str: ...
    def semitones(self) -> int: ...
    def degree(self) -> int: ...
    def octave(self) -> int: ...
    def simple(self) -> "Interval": ...
    def is_compound(self) -> bool: ...
    def invert(self) -> "Interval": ...
    def consonance(self, include_fourth: bool = False) -> str: ...
    def is_consonant(self, include_fourth: bool = False) -> bool: ...
    def full_name(self) -> str: ...
    def full_name_pt(self) -> str: ...
    def __add__(self, other: "Interval") -> "Interval": ...
    def __sub__(self, other: "Interval") -> "Interval": ...
    @overload
    def __eq__(self, other: "Interval") -> bool: ...
    @overload
    def __eq__(self, other: int) -> bool: ...
    def __eq__(self, other: object) -> bool: ...
    @overload
    def __ne__(self, other: "Interval") -> bool: ...
    @overload
    def __ne__(self, other: int) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __hash__(self) -> int: ...
    def __lt__(self, other: "Interval") -> bool: ...
    def __le__(self, other: "Interval") -> bool: ...
    def __gt__(self, other: "Interval") -> bool: ...
    def __ge__(self, other: "Interval") -> bool: ...
    def __int__(self) -> int: ...

class ChordComparison:
    common_notes: List[Note]
    exclusive_a: List[Note]
    exclusive_b: List[Note]
    root_distance: int
    root_direction: int
    same_quality: bool
    same_size: bool
    common_intervals: List[str]
    enharmonic: bool
    subset: str
    voice_leading: int
    transformation: str
    inversion: bool
    interval_vector_a: List[int]
    interval_vector_b: List[int]
    same_interval_vector: bool
    transposition: int
    dissonance_a: float
    dissonance_b: float
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...

class Chord:
    @overload
    def __init__(self, name: str) -> None: ...
    @overload
    def __init__(self, name: List[str]) -> None: ...
    @overload
    def __init__(self, name: List[Note]) -> None: ...
    def name(self) -> str: ...
    def root(self) -> Note: ...
    def type(self) -> str: ...
    def notes(self) -> List[Note]: ...
    def formal_notes(self) -> List[Note]: ...
    def intervals(self) -> List[Interval]: ...
    def interval_labels(self) -> List[str]: ...
    def size(self) -> int: ...
    def contains(self, note: Note) -> bool: ...
    def compare(self, other: "Chord") -> ChordComparison: ...
    def transpose(self, semitones: int) -> "Chord": ...
    def __add__(self, semitones: int) -> "Chord": ...
    def __sub__(self, semitones: int) -> "Chord": ...
    @staticmethod
    def identify(note_names: List[str]) -> "Chord": ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __hash__(self) -> int: ...
    def __len__(self) -> int: ...
    def __contains__(self, note: Union[Note, str]) -> bool: ...
    def __iter__(self) -> Iterator[Note]: ...
    def __reversed__(self) -> Iterator[Note]: ...
    @overload
    def __getitem__(self, index: int) -> Note: ...
    @overload
    def __getitem__(self, index: slice) -> List[Note]: ...

class Scale:
    @overload
    def __init__(self, tonic: str, type: ScaleType, modality: Modality = ...) -> None: ...
    @overload
    def __init__(self, tonic: str, type: str, modality: str = "diatonic") -> None: ...
    def tonic(self) -> Note: ...
    def type(self) -> ScaleType: ...
    def modality(self) -> Modality: ...
    def parent(self) -> ScaleType: ...
    def mode_number(self) -> int: ...
    def is_pentatonic(self) -> bool: ...
    def mode_name(self) -> str: ...
    def quality(self) -> str: ...
    def brightness(self) -> int: ...
    def colors(self, reference: Union["Scale", str]) -> List[Note]: ...
    def notes(self) -> List[Note]: ...
    def formal_notes(self) -> List[Note]: ...
    def degree(self, *degrees: int) -> Note: ...
    def walk(self, start: int, *steps: int) -> Note: ...
    def size(self) -> int: ...
    def contains(self, note: Note) -> bool: ...
    def degree_of(self, note: Note) -> Optional[int]: ...
    def mode(self, degree_or_name: Union[int, str]) -> "Scale": ...
    def pentatonic(self) -> "Scale": ...
    def signature(self) -> int: ...
    def relative(self) -> "Scale": ...
    def parallel(self) -> "Scale": ...
    def neighbors(self) -> Tuple["Scale", "Scale"]: ...
    def mask(self) -> List[int]: ...
    @staticmethod
    def identify(notes: Union[List[str], List[Note]]) -> "Scale": ...
    @staticmethod
    def parse_type(name: str) -> ScaleType: ...
    @staticmethod
    def parse_modality(name: str) -> Modality: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __len__(self) -> int: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __hash__(self) -> int: ...
    def __contains__(self, note: Union[Note, str]) -> bool: ...
    def __iter__(self) -> Iterator[Note]: ...
    @overload
    def __getitem__(self, index: int) -> Note: ...
    @overload
    def __getitem__(self, index: slice) -> List[Note]: ...
    def __reversed__(self) -> Iterator[Note]: ...

class HarmonicFunction(Enum):
    Tonic: int
    Subdominant: int
    Dominant: int
    @property
    def short(self) -> str: ...

class BorrowedInfo:
    scale_type: str
    degree: int
    function: HarmonicFunction
    role: str
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...

class PivotInfo:
    tonic: str
    scale_type: str
    degree_a: int
    degree_b: int
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...

class FieldComparison:
    degree_a: Optional[int]
    degree_b: Optional[int]
    function_a: Optional[HarmonicFunction]
    function_b: Optional[HarmonicFunction]
    role_a: Optional[str]
    role_b: Optional[str]
    degree_distance: Optional[int]
    same_function: Optional[bool]
    relative: bool
    progression: bool
    root_motion: str
    secondary_dominant: str
    applied_diminished: str
    diatonic_a: bool
    diatonic_b: bool
    borrowed_a: Optional[BorrowedInfo]
    borrowed_b: Optional[BorrowedInfo]
    pivot: List[PivotInfo]
    tritone_sub: bool
    chromatic_mediant: str
    foreign_a: List[Note]
    foreign_b: List[Note]
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...

class Field:
    def __init__(self, tonic: str, type: str) -> None: ...
    def tonic(self) -> Note: ...
    def scale(self) -> Scale: ...
    def chords(self) -> List[Chord]: ...
    def sevenths(self) -> List[Chord]: ...
    def chord(self, degree: int) -> Chord: ...
    def seventh(self, degree: int) -> Chord: ...
    @overload
    def applied(self, function: str, target: int) -> Chord: ...
    @overload
    def applied(self, function: str, target: str) -> Chord: ...
    @overload
    def applied(self, function: int, target: int) -> Chord: ...
    def applied(self, function: Union[str, int], target: Union[str, int]) -> Chord: ...
    @overload
    def function(self, degree_or_chord: int) -> HarmonicFunction: ...
    @overload
    def function(self, degree_or_chord: str) -> Optional[HarmonicFunction]: ...
    @overload
    def function(self, degree_or_chord: Chord) -> Optional[HarmonicFunction]: ...
    def function(self, degree_or_chord: Union[int, str, Chord]) -> Optional[HarmonicFunction]: ...
    @overload
    def role(self, degree_or_chord: int) -> str: ...
    @overload
    def role(self, degree_or_chord: str) -> Optional[str]: ...
    @overload
    def role(self, degree_or_chord: Chord) -> Optional[str]: ...
    def role(self, degree_or_chord: Union[int, str, Chord]) -> Optional[str]: ...
    def compare(self, a: Chord, b: Chord) -> FieldComparison: ...
    def signature(self) -> int: ...
    def relative(self) -> "Field": ...
    def parallel(self) -> "Field": ...
    def neighbors(self) -> Tuple["Field", "Field"]: ...
    def size(self) -> int: ...
    @staticmethod
    def identify(items: Union[List[str], List[Note], List[Chord]]) -> "Field": ...
    @staticmethod
    def deduce(items: Union[List[str], List[Note], List[Chord]], limit: int = 10) -> List["FieldMatch"]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __len__(self) -> int: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __hash__(self) -> int: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __contains__(self, chord: Union[Chord, str]) -> bool: ...
    def __iter__(self) -> Iterator[Chord]: ...
    def __reversed__(self) -> Iterator[Chord]: ...
    @overload
    def __getitem__(self, index: int) -> Chord: ...
    @overload
    def __getitem__(self, index: slice) -> List[Chord]: ...
    @overload
    def __getitem__(self, roman: str) -> Chord: ...
    @overload
    def __getitem__(self, applied: Tuple[str, str]) -> Chord: ...
    @overload
    def __getitem__(self, applied: Tuple[str, int]) -> Chord: ...
    @overload
    def __getitem__(self, applied: Tuple[int, int]) -> Chord: ...

class FieldMatch:
    field: Field
    score: float
    matched: int
    total: int
    roles: List[str]
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...

class Tradition:
    name: str
    description: str
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Schema:
    name: str
    description: str
    branches: List[str]
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class HarmonicPath:
    id: int
    branch: str
    chord: Chord
    interval_labels: List[str]
    note_names: List[str]
    def __repr__(self) -> str: ...

class Tree:
    @overload
    def __init__(self, tonic: str, type: ScaleType, tradition: str) -> None: ...
    @overload
    def __init__(self, tonic: str, type: str, tradition: str) -> None: ...
    def tonic(self) -> Note: ...
    def type(self) -> ScaleType: ...
    def tradition(self) -> Tradition: ...
    def branches(self) -> List[str]: ...
    def paths(self, branch_origin: str) -> List[HarmonicPath]: ...
    def shortest_path(self, from_: str, to: str) -> List[str]: ...
    def is_valid(self, branches: List[str]) -> bool: ...
    def function(self, branch: str) -> HarmonicFunction: ...
    def branches_with_function(self, func: HarmonicFunction) -> List[str]: ...
    def schemas(self) -> List[Schema]: ...
    def to_dot(self, show_functions: bool = False) -> str: ...
    def to_mermaid(self) -> str: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class ProgressionMatch:
    tradition: str
    schema: str
    score: float
    matched: int
    total: int
    branches: List[str]
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...

class ProgressionRoute:
    next: str
    tradition: str
    schema: str
    path: List[str]
    confidence: float
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...

class Progression:
    @overload
    def __init__(self, tonic: str, type: ScaleType) -> None: ...
    @overload
    def __init__(self, tonic: str, type: str) -> None: ...
    def tonic(self) -> Note: ...
    def type(self) -> ScaleType: ...
    @staticmethod
    def traditions() -> List[Tradition]: ...
    def tree(self, tradition: str) -> Tree: ...
    def identify(self, branches: List[str]) -> ProgressionMatch: ...
    def deduce(self, branches: List[str], limit: int = 10) -> List[ProgressionMatch]: ...
    def predict(self, branches: List[str], tradition: str = "") -> List[ProgressionRoute]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Duration:
    @overload
    def __init__(self, name: str, dots: int = 0, tuplet: int = 0) -> None: ...
    @overload
    def __init__(self, numerator: int, denominator: int) -> None: ...
    def name(self) -> str: ...
    def dots(self) -> int: ...
    def tuplet(self) -> int: ...
    def beats(self, reference_value: float = 1.0) -> float: ...
    def rational(self) -> Tuple[int, int]: ...
    def numerator(self) -> int: ...
    def denominator(self) -> int: ...
    def midi_ticks(self, ppqn: int = 480) -> int: ...
    @staticmethod
    def from_ticks(ticks: int, ppqn: int = 480) -> "Duration": ...
    @staticmethod
    def standard_names() -> List[str]: ...
    def __add__(self, other: "Duration") -> "Duration": ...
    def __mul__(self, factor: float) -> "Duration": ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __lt__(self, other: "Duration") -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Tempo:
    @overload
    def __init__(self, bpm: float) -> None: ...
    @overload
    def __init__(self, marking: str) -> None: ...
    def bpm(self) -> float: ...
    def marking(self) -> str: ...
    def seconds(self, duration: Duration) -> float: ...
    def ms_per_beat(self) -> float: ...
    def microseconds_per_beat(self) -> int: ...
    @staticmethod
    def bpm_to_marking(bpm: float) -> str: ...
    @staticmethod
    def marking_to_bpm(marking: str) -> float: ...
    @staticmethod
    def from_microseconds(usec: int) -> "Tempo": ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __float__(self) -> float: ...
    def __int__(self) -> int: ...

class TimeSignature:
    def __init__(self, beats_per_bar: int, beat_unit: int) -> None: ...
    def beats_per_bar(self) -> int: ...
    def beat_unit(self) -> int: ...
    def signature(self) -> Tuple[int, int]: ...
    def bar_duration(self) -> Duration: ...
    def classification(self) -> str: ...
    def common_name(self) -> str: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class NoteEvent:
    def __init__(self, note: Note, duration: Duration, octave: int = 4) -> None: ...
    def note(self) -> Note: ...
    def octave(self) -> int: ...
    def midi_number(self) -> int: ...
    def duration(self) -> Duration: ...
    def frequency(self, tuning: float = 440.0) -> float: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class ChordEvent:
    def __init__(self, chord: Chord, duration: Duration, octave: int = 4) -> None: ...
    def chord(self) -> Chord: ...
    def octave(self) -> int: ...
    def duration(self) -> Duration: ...
    def note_events(self) -> List[NoteEvent]: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Rest:
    def __init__(self, duration: Duration) -> None: ...
    def duration(self) -> Duration: ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Sequence:
    def __init__(self, tempo: Tempo = ..., time_signature: TimeSignature = ...) -> None: ...
    def tempo(self) -> Tempo: ...
    def set_tempo(self, tempo: Tempo) -> None: ...
    def time_signature(self) -> TimeSignature: ...
    def set_time_signature(self, ts: TimeSignature) -> None: ...
    def add(self, event: Union[NoteEvent, ChordEvent, Rest]) -> None: ...
    def remove(self, index: int) -> None: ...
    def clear(self) -> None: ...
    def total_duration(self) -> float: ...
    def total_seconds(self) -> float: ...
    def bar_count(self) -> int: ...
    def transpose(self, semitones: int) -> "Sequence": ...
    def to_midi(self, path: Union[str, Path], ppqn: int = 480) -> None: ...
    @staticmethod
    def from_midi(path: Union[str, Path]) -> "Sequence": ...
    def play(self, *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def to_wav(self, path: Union[str, Path], *, octave: int = 4, duration: float = 0.5, waveform: Union[Waveform, str] = ..., amplitude: float = 0.8, envelope: Optional[Envelope] = None, strum: float = 0.03, gap: float = 0.05, tuning: float = 440.0, sample_rate: int = 44100) -> None: ...
    def __len__(self) -> int: ...
    @overload
    def __getitem__(self, index: int) -> Union[NoteEvent, ChordEvent, Rest]: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class PianoKey:
    midi: int
    octave: int
    note: str
    white: bool
    position: int
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class VoicingStyle(Enum):
    Close: int
    Open: int
    Shell: int

class PianoVoicing:
    keys: List[PianoKey]
    style: VoicingStyle
    chord_name: str
    inversion: int
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Piano:
    def __init__(self, num_keys: int = 88) -> None: ...
    def num_keys(self) -> int: ...
    def lowest(self) -> PianoKey: ...
    def highest(self) -> PianoKey: ...
    def in_range(self, midi: int) -> bool: ...
    def key(self, note: Note, octave: int = 4) -> PianoKey: ...
    def keys(self, note: Note) -> List[PianoKey]: ...
    def voicing(self, chord: Chord, octave: int = 4, style: VoicingStyle = ...) -> PianoVoicing: ...
    def voicings(self, chord: Chord, octave: int = 4) -> List[PianoVoicing]: ...
    def scale_keys(self, scale: Scale, octave: int = 4) -> List[PianoKey]: ...
    def note_at(self, midi: int) -> Note: ...
    def identify(self, midi_numbers: List[int]) -> Chord: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class MusicXML:
    @staticmethod
    def note(note: Note, octave: int = 4, type: str = "quarter") -> str: ...
    @staticmethod
    def chord(chord: Chord, octave: int = 4, type: str = "whole") -> str: ...
    @staticmethod
    def scale(scale: Scale, octave: int = 4, type: str = "quarter") -> str: ...
    @staticmethod
    def field(field: Field, octave: int = 4, type: str = "whole") -> str: ...
    @staticmethod
    def sequence(sequence: Sequence) -> str: ...
    @staticmethod
    def write(xml: str, path: str) -> None: ...

class Layout(Enum):
    Vertical: int
    Horizontal: int
    Grid: int

class PianoSVG:
    @staticmethod
    def note(piano: Piano, note: Note, octave: int = 4, compact: bool = False) -> str: ...
    @staticmethod
    def chord(piano: Piano, chord: Chord, octave: int = 4, style: VoicingStyle = ..., compact: bool = False) -> str: ...
    @staticmethod
    def scale(piano: Piano, scale: Scale, octave: int = 4, compact: bool = False) -> str: ...
    @staticmethod
    def keys(piano: Piano, highlighted: List[PianoKey], title: str = "", compact: bool = False) -> str: ...
    @staticmethod
    def voicing(piano: Piano, voicing: PianoVoicing, compact: bool = False) -> str: ...
    @staticmethod
    def midi(piano: Piano, midi_numbers: List[int], compact: bool = False) -> str: ...
    @staticmethod
    def field(piano: Piano, field: Field, octave: int = 4, layout: Layout = ..., sevenths: bool = False) -> str: ...
    @staticmethod
    def progression(piano: Piano, field: Field, branches: List[str], octave: int = 4, layout: Layout = ...) -> str: ...
    @staticmethod
    def write(svg: str, path: str) -> None: ...

class FretPosition:
    string: int
    fret: int
    midi: int
    note: str
    octave: int
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class StringAction(Enum):
    Open: int
    Fretted: int
    Muted: int

class StringState:
    string: int
    action: StringAction
    fret: int
    finger: int
    def __repr__(self) -> str: ...

class Tuning:
    name: str
    open_midi: List[int]
    num_frets: int
    def __repr__(self) -> str: ...

class Fingering:
    strings: List[StringState]
    chord_name: str
    barre: int
    base_fret: int
    capo: int
    midi_notes: List[int]
    def to_dict(self) -> Dict[str, Any]: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Fretboard:
    def __init__(self, name: str, open_midi: List[int], num_frets: int = 19) -> None: ...
    @staticmethod
    def cavaquinho(num_frets: int = 17) -> "Fretboard": ...
    @staticmethod
    def violao(num_frets: int = 19) -> "Fretboard": ...
    @staticmethod
    def bandolim(num_frets: int = 17) -> "Fretboard": ...
    def name(self) -> str: ...
    def num_strings(self) -> int: ...
    def num_frets(self) -> int: ...
    def tuning(self) -> Tuning: ...
    def position(self, string: int, fret: int) -> FretPosition: ...
    def positions(self, note: Note) -> List[FretPosition]: ...
    def note_at(self, string: int, fret: int) -> Note: ...
    def midi_at(self, string: int, fret: int) -> int: ...
    @overload
    def scale_positions(self, scale: Scale) -> List[FretPosition]: ...
    @overload
    def scale_positions(self, scale: Scale, fret_lo: int, fret_hi: int) -> List[FretPosition]: ...
    def fingering(self, chord: Chord, position: int = 0) -> Fingering: ...
    def fingerings(self, chord: Chord, max_results: int = 5) -> List[Fingering]: ...
    def identify(self, string_frets: List[Tuple[int, int]]) -> Chord: ...
    def capo(self, fret: int) -> "Fretboard": ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...

class Orientation(Enum):
    Horizontal: int
    Vertical: int

class Handedness(Enum):
    RightHanded: int
    LeftHanded: int

class FretboardSVG:
    @staticmethod
    def chord(fretboard: Fretboard, chord: Chord, position: int = 0, orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def fingering(fretboard: Fretboard, fingering: Fingering, orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def scale(fretboard: Fretboard, scale: Scale, fret_lo: int = 0, fret_hi: int = 12, orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def positions(fretboard: Fretboard, highlighted: List[FretPosition], title: str = "", orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def note(fretboard: Fretboard, note: Note, orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def field(fretboard: Fretboard, field: Field, layout: Layout = ..., orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def progression(fretboard: Fretboard, field: Field, branches: List[str], layout: Layout = ..., orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def full(fretboard: Fretboard, orientation: Orientation = ..., handedness: Handedness = ...) -> str: ...
    @staticmethod
    def write(svg: str, path: str) -> None: ...
